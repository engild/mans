...
| -e                            | 通过key=value模式设置变量，也可以使用@filename的方式指定yaml或者json文件来设置变量
| -l SUBSET, --limit SUBSET     | 进一步限制哪些主机可以执行任务
| --list-host                   | 仅输出所匹配主机列表
| --list-tags                   | 列出所有可用标签
| --list-tasks                  | 列出将要执行的所有任务
| -t                            | 只运行带有这些 tag 标记的 play 和 task
| -u REMOTE_USER                | 以该用户身份连接
| -k                            | 询问 REMOTE_USER 的密码
| --become-user BECOME_USER     | 指定 BECOME_USER，默认为root
| -K                            | 访问 BECOME_USER 的密码
...
# 限制只对 host01 运行
ansible-playbook -i inventories/test/ playbook.yaml -l host01
```
